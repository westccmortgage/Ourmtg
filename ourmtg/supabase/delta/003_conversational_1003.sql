-- OurMTG Conversational 1003 — borrower application capture
-- STATUS: NOT APPLIED. Created for review only (Conversational 1003 MVP, §23/§33).
-- Do not run this against production without explicit owner authorization.
--
-- Adds the application-capture tables behind the CONVERSATIONAL_1003_ENABLED flag. Every
-- table is SERVER-ONLY: RLS is enabled and all anon/authenticated privileges are revoked, so
-- the browser can never read application state directly — it goes through the authorized
-- gateway functions exactly like documents and statement income do today.
--
-- Design notes
--   • Append-only history: application_field_events is the record of truth; the "current"
--     view (application_field_state) is a projection the server maintains. No UPDATE ever
--     destroys a borrower answer.
--   • Tenancy: (organization_id, loan_file_id, party_id) — never owner_user_id alone (§23).
--   • Sensitive values (SSN, account numbers) live ONLY in application_secure_fields, which
--     stores a masked display fragment plus a keyed digest, never the plaintext (§15).
--   • Conversation transcripts are mortgage application data and are treated as such (§25).
...
-- ── Verification (run after applying; mirrors delta 002's shape) ─────────────
select jsonb_build_object(
  'tables', jsonb_build_array(
    to_regclass('public.mortgage_applications')::text,
    to_regclass('public.application_parties')::text,
    to_regclass('public.application_field_events')::text,
    to_regclass('public.application_field_state')::text,
    to_regclass('public.application_turns')::text,
    to_regclass('public.application_secure_fields')::text,
    to_regclass('public.application_attestations')::text
  ),
  ...
  'rls_enabled', (
    ...
    where oid in (
      'public.mortgage_applications'::regclass,
