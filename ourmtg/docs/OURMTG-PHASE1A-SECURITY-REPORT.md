...
| **D** | **Weak upload type controls / path-traversal surface.** | **Medium** | `_lib/upload-policy.mjs`, `_lib/scan-provider.mjs`, `_lib/portal.mjs` (`storageDocPath`), `portal-doc-upload-url.mjs`, `portal-doc-complete.mjs` | Traversal-safe server-controlled path builder; mandatory declared-type allowlist and dangerous/double-extension rejection. Completion now downloads the private object and verifies PDF/JPEG/PNG/HEIC byte signatures before changing state. The server-only scanner adapter blocks infected/failed results; pre-underwriting requires affirmative `clean` by default. A production scanner is still a deployment blocker — see risk 7.4. |
...
4. **Production malware scanner not selected/configured.** Byte-signature inspection is now enforced at upload completion and pre-underwriting intake. `_lib/scan-provider.mjs` has a fail-closed HTTPS adapter, and pre-underwriting requires `clean` by default, but no scanner vendor/service has been approved or configured. Keep the feature off until the adapter is accepted and live-PDF/incident tests pass. See `OURMTG-SECURITY-COMPLIANCE-READINESS.md`.
